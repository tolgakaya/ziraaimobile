...
/// Farmer Payment Screen for subscription purchases
///
/// Flow:
/// 1. Initialize payment with tier and duration
...
class _FarmerPaymentScreenState extends State<FarmerPaymentScreen> {
  ...
  Future<void> _initializePayment() async {
    ...
    try {
      // Initialize payment with farmer subscription flow
      final response = await _paymentService.initializeFarmerPayment(
  ...
  Future<void> _verifyPayment(String paymentToken) async {
    ...
    try {
      ...
      if (verifyResponse.isSuccess) {
        // Payment successful
        final farmerResult = verifyResponse.farmerResult;
        _showSuccessDialog(farmerResult);
  ...
  void _showSuccessDialog(FarmerSubscriptionResult? result) {
    showDialog(
      context: context,
      barrierDismissible: false,
          ...
          content: Column(
            ...
            children: [
              ...
              if (result != null) ...[
                ...
                Text('Başlangıç: ${_formatDate(result.startDate)}'),
                const SizedBox(height: 8),
                Text('Bitiş: ${_formatDate(result.endDate)}'),
                const SizedBox(height: 8),
                Text('Abonelik ID: ${result.subscriptionId}'),
              ],
              const SizedBox(height: 16),
              const Text(
                'Artık premium özelliklerden yararlanabilirsiniz!',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Abonelik Ödeme'),
        leading: IconButton(
