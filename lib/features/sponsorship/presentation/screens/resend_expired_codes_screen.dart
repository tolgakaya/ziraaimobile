...
class ResendExpiredCodesScreen extends StatefulWidget {
  final SponsorDashboardSummary dashboardSummary;

  const ResendExpiredCodesScreen({
    super.key,
    required this.dashboardSummary,
  });

  @override
  State<ResendExpiredCodesScreen> createState() => _ResendExpiredCodesScreenState();
}

class _ResendExpiredCodesScreenState extends State<ResendExpiredCodesScreen> {
  ...
  Future<void> _loadExpiredCodes({bool loadMore = false}) async {
    ...
    try {
      // IMPORTANT: Use getSentExpiredCodes() to get codes that were sent but expired
      // Filters: DistributionDate IS NOT NULL, ExpiryDate < NOW, IsUsed = false
      final result = await _sponsorService.getSentExpiredCodes(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Süresi Dolmuş Kodları Tekrar Gönder'),
  ...
  Widget _buildMainContent() {
    ...
    return RefreshIndicator(
      onRefresh: () => _loadExpiredCodes(loadMore: false),
            ...
            if (_hasMoreCodes && !_isLoadingMore)
              Container(
                ...
                child: OutlinedButton.icon(
                  onPressed: () => _loadExpiredCodes(loadMore: true),
  ...
  Future<void> _sendLinks() async {
    ...
    try {
      ...
      // DEBUG LOG
      print('🔄 RESEND EXPIRED CODES: Sending ${allRecipients.length} links via $channelName');
      print('🔄 RESEND EXPIRED CODES: Total packages: ${_packageRecipients.entries.where((e) => e.value.isNotEmpty).length}');
      print('🔄 RESEND EXPIRED CODES: Selected codes: $allCodes');
      print('🔄 RESEND EXPIRED CODES: Recipients: ${allRecipients.map((r) => '${r.name} (${r.phone})').toList()}');
      ...
      // DEBUG LOG
      print('🔄 RESEND EXPIRED CODES: Response received - success: ${response.success}');
      print('🔄 RESEND EXPIRED CODES: Success count: ${response.data?.successCount}');
      print('🔄 RESEND EXPIRED CODES: Failure count: ${response.data?.failureCount}');
      ...
      print('🔄 RESEND EXPIRED CODES: ERROR - $e');
      if (mounted) {
